...
function onSpotifyInfoReceived(songRequest) {
  console.log('onSpotifyInfoReceived for songRequest: ', songRequest.id)
...
function onSpotifyInfoFailed(err) {
  logAction('onSpotifyInfoFailed', err)
...
function onYouTubeInfoReceived(songRequest) {
  logAction('onYouTubeInfoReceived', songRequest)
...
function onYouTubeInfoFailed(songRequest) {
  logAction('onYouTubeInfoFailed', songRequest)
...
function logAction(actionName, songRequest) {
  var name = songRequest.spotify_id
  if (songRequest.spotify_info && songRequest.spotify_info.name) {
    name = songRequest.spotify_info.name
  }
  console.log(`${actionName} fired for ${name}`)
}
